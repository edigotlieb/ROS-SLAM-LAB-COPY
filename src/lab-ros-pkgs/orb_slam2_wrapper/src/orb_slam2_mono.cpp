...
OrbSlam2Mono::OrbSlam2Mono(ros::NodeHandle nh)
    : nh_(nh),
      path_vocabulary_("/lab/3rdparty/ORB_SLAM2/Vocabulary/ORBvoc.txt"),
...
OrbSlam2Mono::~OrbSlam2Mono() {
  mono_slam_->Shutdown();
}

void OrbSlam2Mono::readNodeParams() {
  ...
  nh_.param<std::string>("pose_topic", pose_topic_, "orb_slam2_pose");// name in launch file, name of variable, default value
...
void OrbSlam2Mono::internalInit() {
  variance_[X] = var_x_;
  variance_[Y] = var_y_;
  variance_[Z] = var_z_;
  ...
  mono_slam_ =
      new ORB_SLAM2::System(path_vocabulary_, path_settings_, ORB_SLAM2::System::MONOCULAR, use_viewer_);
}

void OrbSlam2Mono::initializeSubscribers() {
  image_sub_ = nh_.subscribe(camera_topic_, 1, &OrbSlam2Mono::GrabMono, this);

}

void OrbSlam2Mono::initializePublishers() {
  pose_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>(pose_topic_, 1);
}

void OrbSlam2Mono::GrabMono(const sensor_msgs::ImageConstPtr &msg) {
...
void OrbSlam2Mono::publishPose(cv::Mat Tcw, cv_bridge::CvImageConstPtr cv_ptr) {
